...
NABoolean handleCharsetPerfix(char* source,
                              CollHeap * heap,
                              Lng32& err_code,
                               Descriptor * input_desc)
{
    NAString* pTempStr_test = NULL;
    pTempStr_test = unicodeToChar((wchar_t*)&source, input_desc->getVarDataLength(1),
                             static_cast<Lng32>(CharInfo::UTF8), heap);
    return TRUE;
    char perfix[MAX_CHAR_SET_STRING_LENGTH];
    Lng32 endIndex = MAX_CHAR_SET_STRING_LENGTH;
    NAWchar wCharBuffer[1000];
    if (source)
    {
        Lng32 perfix_beg = 0;
        Lng32 perfix_end = 0;
...
//            }
            ...
            {
                NABoolean containsOnlyASCII = na_wcs_has_only_ascii_chars((wchar_t*)&source[2], 8);
            ...
            while(*(source+valueEnd))
            {
                if (source[valueEnd] == '\'')
                {
                    ...
                    // cs == unicode
                   // Lng32 len = LocaleStringToUnicode(cs, pTempStr->data(), pTempStr->length(), wCharBuffer, 1000, TRUE);
//                    memcpy(source, wCharBuffer, len);
                    // 灵异事件发生地

//                    if (input_descriptor)
//                    {
//                        Lng32 retcode;
//                        retcode = SQL_EXEC_SETDESCITEM(input_descriptor,
//                                                       entry,
//                                                       SQLDESC_VAR_DATA,
//                                                       (Long)(pTempStr->data()),
//                                                       0);
     ...
     /*IN*/ SQLCLI_PTR_PAIRS output_ptr_pairs[])
{
  ...
  if (tasks & CLI_PT_GET_INPUT_DESC)
      ...
      else
    {
      ...
      if (input_desc && stmt->getRootTdb()->odbcQuery())
      {
          char *source  = NULL;
          Lng32 length = 0;
          const char* cs = NULL;
          Lng32 err_code;
          for (Lng32 entry = 1; entry <= input_desc->getUsedEntryCount(); entry++)
          {
             source = input_desc->getVarData(entry);
             length = input_desc->getVarDataLength(entry);
             cs = input_desc->getVarDataCharSet(entry);
            // cs = input_desc->getVarDataCharSet(entry);
             if (!handleCharsetPerfix(source, cliGlobals->currContext()->exCollHeap(),  err_code, input_desc))
             {
                diags << DgSqlCode(-err_code);
                return SQLCLI_ReturnCode(&currContext, -err_code);
             }
